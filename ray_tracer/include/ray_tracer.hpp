...
class ray_tracer
{
 ...
 private:
  ...
  rtc_hot auto compute_intersection(const rtc::math_ray&, const std::uint32_t, const rtc_float) const noexcept
      -> std::tuple<rtc_float, bool>;
};

template <typename T>
auto ray_tracer<T>::compute_intersection(const rtc::math_ray& ray,
                                         const std::uint32_t triangle_index,
                                         const rtc_float ray_triangle_hit_value) const noexcept
    -> std::tuple<rtc_float, bool>
{
  ...
  const auto t_temp = create_plane_test(scene->points, scene->triangles, triangle_index).intersection_value(ray);
