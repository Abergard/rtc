...
void brs::load_points(const boost::property_tree::ptree& tree)
{
  ...
  for (const auto& n : nodes)
  {
    if (n.first == "point")
    {
      points.emplace_back(n.second.get<rtc_float>("<xmlattr>.x"), n.second.get<rtc_float>("<xmlattr>.y"),
...
void brs::load_triangles(const boost::property_tree::ptree& tree)
{
  ...
  for (const auto& n : nodes)
  {
    if (n.first == "triangle")
    {
      triangles.emplace_back(n.second.get<std::uint32_t>("<xmlattr>.v1"), n.second.get<std::uint32_t>("<xmlattr>.v2"),
...
void brs::load_lights(const boost::property_tree::ptree& tree)
{
  ...
  for (const auto& n : nodes)
  {
    if (n.first == "light")
    {
      ...
      l.position = rtc::math_point{n.second.get<rtc_float>("position.<xmlattr>.x"),
    ...
    {
      ambient = rtc::color{n.second.get<rtc_float>("color.r"), n.second.get<rtc_float>("color.g"),
                           n.second.get<rtc_float>("color.b")};
    }
  }
}
  ...
  for (const auto& n : nodes)
  {
    if (n.first == "attribute")
    {
      ...
      for (auto& nn : n.second.get_child(""))
      {
        ...
        {
          sm.material_color =
              rtc::color{nn.second.get<rtc_float>("r"), nn.second.get<rtc_float>("g"), nn.second.get<rtc_float>("b")};
        }
      }
...
void brs::load_camera_info(const boost::property_tree::ptree& tree)
{
  ...
  for (const auto& n : tree.get_child("model.observers"))
  {
    if (n.first == "camera")
    {
      ...
      c.view_point = {n.second.get<rtc_float>("position.<xmlattr>.x"), n.second.get<rtc_float>("position.<xmlattr>.y"),
                      n.second.get<rtc_float>("position.<xmlattr>.z")};
      ...
      for (const auto& nn : n.second.get_child("screen"))
      {
        if (nn.first == "corner")
        {
          ...
          }() = {nn.second.get<rtc_float>("<xmlattr>.x"), nn.second.get<rtc_float>("<xmlattr>.y"),
